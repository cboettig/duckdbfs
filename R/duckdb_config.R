...
#'
#' @param conn A database connection object created using the \code{cache_connection} function (default: \code{cache_connection()}).
...
#' @param s3_uploader_max_filesize The maximum filesize for S3 uploader (between 50GB and 5TB, default 800GB).
...
duckdb_s3_config <- function(conn = cached_connection(),
                      s3_access_key_id = NULL, # Sys.getenv("AWS_ACCESS_KEY_ID"),
                      s3_secret_access_key = NULL, #Sys.getenv("AWS_SECRET_ACCESS_KEY"),
                      s3_endpoint = NULL, #Sys.getenv("AWS_S3_ENDPOINT"),
                      s3_region = NULL, #Sys.getenv("AWS_DEFAULT_ENDPOINT"),
